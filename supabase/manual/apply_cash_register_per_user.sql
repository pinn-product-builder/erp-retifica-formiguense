-- Aplicar manualmente no SQL Editor se `db push` não aplicar (histórico divergente).
-- Espelha supabase/migrations/20260505120000_cash_register_per_user.sql
--
...
-- Idempotente: ao rerodar o script, a UNIQUE nova já pode existir.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'cash_closings_org_date_bank_account_key'
