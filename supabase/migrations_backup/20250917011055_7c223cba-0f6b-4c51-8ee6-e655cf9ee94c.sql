...
CREATE INDEX IF NOT EXISTS idx_user_basic_info_user_id ON public.user_basic_info(user_id);
CREATE INDEX IF NOT EXISTS idx_user_basic_info_email ON public.user_basic_info(email);
...
AS $$
  -- Retorna dados básicos disponíveis - email será obtido de user_basic_info quando disponível
  SELECT 
    ou.user_id,
    COALESCE(ubi.email, 'email@example.com') as email,
    COALESCE(COALESCE(ubi.name, p.name), 'Nome não disponível') as name,
    ou.created_at
